...
echo "$($PSQL "SELECT c.course FROM students AS s FULL JOIN majors USING (major_id) FULL JOIN majors_courses USING (major_id) FULL JOIN courses AS c USING (course_id) GROUP BY c.course HAVING COUNT(s.student_id) = 1 ORDER BY c.course")"
